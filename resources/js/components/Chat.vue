		...
		<div v-if="checkProfile" class="row">
			<div class="col-sm-12 col-md-3 col-lg-3 overflow-auto" :style="!chatResize ? 'height: 100vh' : ''">
				...
				<div v-else>
					...
					<div class="collapse" :class="!chatResize ? 'show' : ''" id="chat-sidebar">
			...
			<div class="col-sm-12 col-md-9 col-lg-9">
				<div id="id-chat" class="overflow-auto mb-2" style="height: 100vh">
					...
					<div v-if="selected" v-for="m in msg">
						<p v-if="m.user_id === $auth.user().id" class="text-right"><img class="img-fluid rounded-circle" width="40" height="40" :src="$auth.user().avatar || sauce+'/img/default.png'" /> {{ $auth.user().name }} <small class="text-muted">{{ dateFill(m.created_at) }}</small></p>
						...
						<p class="text-break" :class="m.user_id === $auth.user().id ? 'text-right' : ''">{{ m.msg }}</p>
						<hr/>
					</div>
				</div>
				<form v-if="selected" @submit.prevent="send">
					<div class="input-group">
						<input class="form-control" type="text" v-model="msgChat" placeholder="Isi pesan..">
...
<script>
	export default{
		...
		methods: {
			...
			openChat(id, key, pass = false){
				if(this.teacher[key].name != this.select.name || pass){
					this.selected = true
					this.select = {
					...
					pass ? $('#icon-loader').hide() : $('#icon-loader').show()
			...
			send(){
				if(this.msgChat != ''){
					...
					axios.post(`/chat/send/${this.$auth.user().id}/${this.id}`, {msg: this.msgChat})
						.then(() => {
							this.msgChat = ''
							this.openChat(this.id, this.key, true)
